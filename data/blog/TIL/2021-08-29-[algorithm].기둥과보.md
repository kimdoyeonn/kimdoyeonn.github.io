...
def solution(n, build_frame):
    ...
    for [x, y, stuff, oper] in build_frame:
        if oper == 0:  # 삭제
            # 자재를 삭제하고 삭제해도 괜찮은지 확인
            # 괜찮으면 넘어가고, 안 괜찮으면 다시 설치해줌
            answer.remove([x, y, stuff])
            if not check(answer):
                answer.append([x, y, stuff])
        else:  # 설치
            # 자재를 설치하고 설치할 수 있는지 확인
            # 할 수 있으면 그대로 두고, 안 되면 삭제
            answer.append([x, y, stuff])
            if not check(answer):
                answer.remove([x, y, stuff])
...
구조물이 설치, 삭제될 때마다 리스트 `answer`를 확인하면서 기둥들과 보들이 들어갈 수 있는지 매번 확인하는 방식으로 구현하였다.
